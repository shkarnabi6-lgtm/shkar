 const streams = [
    ...
    function showChannelName(name) {
        // Set the channel name to the #channelName div
        channelNameBox.textContent = name;
        channelNameBox.style.opacity = 1;  // Ensure it stays visible
    }
    ...
    function loadStream(id) {
        ...
        // Stop any active media before loading new stream
        if (frame.src) {
            frame.src = "";  // Stop any iframe stream
        }

        if (hls) {
            hls.destroy();  // Stop any video playback
            hls = null;
        }
